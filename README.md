...
- для EPIC-09 подтверждены PR/main CI; EPIC-02 прошёл local, PR и exact-merge CI после отдельного transport-safe test fix; EPIC-03 live state/quality реализуется в новой ветке от verified `main`;
...
Текущая readiness по независимо проверенным atomic AC: `28/139 = 20.1%` initial release и `28/164 = 17.1%` full roadmap. `EPIC-02` имеет `READY 15/15`; `EPIC-03` начат с denominator `13`; `EPIC-09` имеет `13/14`, а persistent history allowlist (`E09-AC06`) будет доказан вместе с реальной SQLite schema в `EPIC-08`.
