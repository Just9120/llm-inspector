...
func validateObservation(o *domain.Observation) error {
	if id(o.RequestID) == "" || o.StartedAt.IsZero() || !finiteDuration(o.DurationMS) || code(outcomes, o.Outcome) < 0 || errorCode(o.ErrorType) < 0 || code(origins, o.ErrorOrigin) < 0 || code(clients, o.Client) < 0 || code(backends, o.Telemetry.Backend) < 0 || code(loads, o.Telemetry.ModelLoad) < 0 || !validOptionalIdentifier(o.Telemetry.Model) || !o.Runtime.Valid() {
		return ErrInvalid
	}
...
func recordRequest(ctx context.Context, tx *sql.Tx, o *domain.Observation) error {
	...
	args := []any{id(o.RequestID), session, dbTime(o.StartedAt), o.HTTPStatus, code(outcomes, o.Outcome), errorCode(o.ErrorType), code(origins, o.ErrorOrigin), code(clients, o.Client), code(backends, o.Telemetry.Backend), nullable(o.Telemetry.Model), turn, seq, code(loads, o.Telemetry.ModelLoad)}
...
func upsertSession(ctx context.Context, tx *sql.Tx, session string, start time.Time, end *time.Time, client domain.Client, backend domain.Backend, model string) error {
	...
	err := tx.QueryRowContext(ctx, "SELECT client,backend FROM sessions WHERE session_id=?", id(session)).Scan(&oldClient, &oldBackend)
	if err != nil && err != sql.ErrNoRows {
		return err
	}
	if err == nil && (oldClient != code(clients, client) || oldBackend != code(backends, backend)) {
		return ErrInvalid
	}
