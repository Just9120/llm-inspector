...
- Code/tests/runtime: merged baseline содержит loopback gateway, adapters, live/token/context/timing UI, schema v4 operation/history/resource analytics, EPIC-01 diagnostics surface и terminal EPIC-07 explainable diagnostics. Active EPIC-10 candidate adds Windows tray/background lifetime, versioned atomic settings, per-user autostart and typed content-free notification policy. Format, Release build with zero warnings/errors, Unit `53/53` and Windows `58/58` focused tests passed; full local pipeline and exact-revision CI pending.
