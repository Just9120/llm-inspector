...
Фактическое состояние пересчитано с нуля по 164 atomic product AC. Exact merged `main` `ea8498efae98adda684dd88d92e52d3401872a5c` подтверждает EPIC-01 `3/4`, EPIC-02 `15/15`, EPIC-03 `13/13`, EPIC-04 `12/12`, EPIC-05 `8/8`, EPIC-06 `8/8`, EPIC-07 `16/16`, EPIC-08 `18/18` и EPIC-09 `14/14`. Active EPIC-10 candidate локально выполняет `E10-AC01..08` через close-to-tray lifecycle, background history ownership, per-user autostart, independently gated typed notifications, silent delivery и versioned anti-spam policy. `E01-AC01` и другие product AC не кредитуются.
...
| **Initial release total** | **🟦 IN PROGRESS** | **115/139** | **82.7%** | **◐** | **◐** | **◐** | **◐** | **N/A** | **N/A** |
...
| **Full agreed roadmap total** | **🟦 IN PROGRESS** | **115/164** | **70.1%** | **◐** | **◐** | **◐** | **◐** | **—*** | **—*** |
...
- Last recalculation: `2026-09-03T08:41:55Z`.
...
- GitHub Actions: EPIC-04 partial PR/main runs `33696539694`/`33696722298`, EPIC-08 PR/main runs `33702613336`/`33702791561`, EPIC-04 fix PR/main runs `33720248633`/`33720428488`, EPIC-05 PR/main runs `33724914481`/`33725139103`, EPIC-06 PR/main runs `33728471307`/`33728697717`, EPIC-07 PR/main runs `33731824812`/`33732075018`; all completed successfully.
- Code/tests/runtime: merged baseline содержит loopback gateway, adapters, live/token/context/timing UI, schema v4 operation/history/resource analytics, EPIC-01 diagnostics surface и terminal EPIC-07 explainable diagnostics. Active EPIC-10 candidate adds Windows tray/background lifetime, versioned atomic settings, per-user autostart and typed content-free notification policy. Exact SDK `10.0.400` full local pipeline passed: locked normal/RID restores, format, Release build with zero warnings/errors, `186/186` tests with zero skips, clean self-contained `win-x64` publish and smoke `exit 0`; exact-revision CI pending.
- EPIC-09 completion: `14/14`; real SQLite schema/disclosure/privacy corpus confirmed by PR #9 and exact-main CI.
- Initial release readiness: `115/139 = 82.7%` (`EPIC-10 8/8` locally; exact-revision CI pending).
- Full agreed roadmap readiness: `115/164 = 70.1%`.
...
- Active approved Goal: `GOAL-005 IN_PROGRESS`; EPIC-07 delivered through PR [#13](https://github.com/Just9120/llm-inspector/pull/13), merge `ea8498efae98adda684dd88d92e52d3401872a5c`, exact-main CI `33732075018`; active EPIC-10 branch is `codex/goal-005-epic-10` from that base.
