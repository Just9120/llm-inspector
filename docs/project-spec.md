...
- Code/tests/runtime: merged baseline содержит loopback gateway, adapters, live/token/context/timing UI, schema v4 operation/history/resource analytics and EPIC-01 diagnostics surface. Active EPIC-07 candidate adds versioned explainable rules, typed error capture, recurring groups and metadata-only correlation. Exact SDK `10.0.400` Release build passed with zero warnings/errors; focused Unit/Integration/Windows/Privacy suites passed `151/151` with zero skips; full local pipeline and exact-revision CI pending.
