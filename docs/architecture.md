...
Root `main_windows.go`/`host_windows.go` реализуют Wails host: single-instance gate до SQLite initialization, typed facade, native dialogs, WebView2 без автоматической установки, tray-aware hide/exit и bounded shutdown. `frontend/` — пять русских Svelte/TypeScript surfaces (overview/history/analytics/backends/settings), CSP/self-only assets, provenance/quality, progressive disclosure. Background polling останавливается при hide. Local executable smoke проверяет actual WebView2 bridge, навигацию, synthetic history и privacy; не заменяет manual Windows matrix. C# reference ниже остаётся до final cutover.
