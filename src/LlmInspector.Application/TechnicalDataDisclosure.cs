...
public static class TechnicalDataDisclosure
{
    ...
    [
        new(
            ...
            "Generated request ID, pseudonymous operation/session/turn correlation IDs and sequence when explicitly supplied, current stage/evidence, elapsed time, backend-reported progress, estimated ETA with quality, HTTP status, typed error category/origin, outcome, configured backend, explicit client attribution, normalized model identity, technical runtime/configuration identifiers, available/invoked tool counts, normalized tool names, token/context/timing metrics with quality/provenance, model-load classification, and allowlisted backend-specific metrics",
            "Process lifetime only"),
        new(
            "Local SQLite technical history",
            "Pseudonymous request/session/turn/operation IDs, timestamps, durations, status/error classification and origin, configured client/backend, normalized model/tool/GPU identifiers, allowlisted Inspector/framework/OS/adapter/backend/client/model/GPU-driver versions and runtime configuration fingerprint when available, available/invoked tool counts, tool duration quality/provenance, request/stage-correlated host GPU/CPU/RAM resource samples, exact related-process identity when provable, process CPU/RAM/disk counters, gateway traffic byte counters, model-load classification, and allowlisted numeric metrics with quality/provenance",
